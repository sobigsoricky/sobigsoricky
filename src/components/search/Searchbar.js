...
class Searchbar extends Component {
  ...
  countData = (data, dataLen, type) => {
    ...
    data.map((item, i) => {
      ...
      if (type === "provider") {
        item.provider.toLowerCase() in providerCount
          ? (providerCount[item.provider.toLowerCase()] += 1)
          : (providerCount[item.provider.toLowerCase()] = 1)
      } else if (type === "university") {
        item.university.toLowerCase() in universityCount
          ? (universityCount[item.university.toLowerCase()] += 1)
          : (universityCount[item.university.toLowerCase()] = 1)
      } else if (type === "institute") {
        item.institute.toLowerCase() in instituteCount
          ? (instituteCount[item.institute.toLowerCase()] += 1)
          : (instituteCount[item.institute.toLowerCase()] = 1)
      }
      return null
    })
  ...
  loadData1 = () => {
    onGetData(
      "api/user/course/testingCourseData/" +
        (parseInt(this.state.limit) + 500) +
        "/0"
    )
      .then((res) => {
        if (res.data.error == false && res.data.data.length > 0) {
          ...
          this.setState({
            apiData: data,
            limit: parseInt(this.state.limit) + 500,
          })
          const providerCount = {}
          let dataLen = data.length
          this.countData(data, dataLen, "provider")
  ...
  onSearch = (e) => {
        ...
        if (item.university?.toLowerCase() === searchValue) {
          ...
          this.state.apiData.map((i) => {
            if (searchValue === i.university.toLowerCase()) {
              ...
              !subSubjectCount.includes(i.subSubject) &&
                subSubjectCount.push(i.subSubject)
            }
            return null
          })
          ...
          if (item.university) {
            item.university in universityDict
              ? (universityDict[item.university] += 1)
              : (universityDict[item.university] = 1)
          }
          if (this.state.apiData[i].institute) {
            item.institute in instituteDict
              ? (instituteDict[item.institute] += 1)
              : (instituteDict[item.institute] = 1)
          }
      ...
      if (Object.keys(providerDict).length >= 1) {
        ...
        this.setState({
          filterData: {
            ...
            subject: subjectCount.length,
            subSubject: subSubjectCount.length,
          },
        })
    ...
    if (searchValue.length !== 0) {
      ...
      this.setState({ cources: {}, loadinT: false })
    }
  }
  render() {
    ...
    return (
      <>
        <div ref={this.myRef} className={classes.outersearch}>
          {/* <TextField
            ...
            onClick={()=>{this.setState({clickedOutside: false})}}
            InputProps={{
              endAdornment: (
                <InputAdornment>
          ...
          <Paper
            ...
            className={`${classes.root} ${classes.textfield} textfieldsearch`}
          >
            <InputBase
            ...
            <IconButton
              ...
              aria-label='search'
            >
              {this.state.loadinT === true ? (
                <ReactLoading type='bars' color='#5C0090' />
              ) : (
                <SearchIcon />
              )}
            </IconButton>
