...
export default function University(props) {
  ...
  var isQuery = query.get("uni") ? true : false;
  const loadClickHandler = async () => {
    setloadmorbtn(true);
    await onGetData(
      "api/user/university/getDataById/" +
        query.get("uni") +
        "/" +
        parseInt(limit) +
        10 +
  ...
  useEffect(() => {
    if (isQuery == false) {
      onGetData("api/user/university/getAllData")
        .then((response) => {
  ...
  return (
    <>
      {loadingData == true ? (
        <Loading name="University" />
      ) : (
        <div className={clsx(classes.container, classes.sectionDesktop)}>
           
           <Helmet>
                <title>All universities | Coursenator</title>
                <meta name="description" content="We offer online courses from top universities like Harvard University, Michigan University, and Institutes like Google, IBM, Microsoft, etc." />
            </Helmet>
              {" "}
              <Title className={classes.title}>List of Universities</Title>{" "}
                    ...
                    return (
                      ...
                      >
                        <div className={classes.logo}>
                          <Link to={`/university-list/${x.slug}`}>
                            <img
                              src={x.image_path}
